...
def analysis(model_save_path, id2relation, module_name):
    print('=' * 50)
    model = torch.load(model_save_path, map_location=torch.device('cpu'))

    
    w = model['model'][f'{module_name}.diff_w']
    weight = model['model'][f'{module_name}.diff_weights']
    
    print(f"Shape of {module_name} diff_w:", w.shape)
    print(f"Shape of {module_name} diff_weights:", weight.shape)

    T = 3
    n = w.shape[0]  # number of relations
    L = w.shape[2]
...
if __name__ == '__main__':
    ...
    for module_name in ["temporal_reasoning_module", "causal_reasoning_module", "subevent_reasoning_module"]:
        print(f"Analysis for {module_name}:")
        analysis(sys.argv[1], id2relation, module_name)
